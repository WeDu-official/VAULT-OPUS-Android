...
def open_explorer_for_sharables(path_str: Optional[str] = None) -> bool:
    ...
    if 'android' in platform.platform().lower():
        try:
            ...
            # Use DocumentsUI or general file manager intent
            intent = Intent(Intent.ACTION_VIEW)
            # Use file provider or direct path depending on Android version and permissions
            # For simplicity and MANAGE_EXTERNAL_STORAGE, we try to open it as a directory
            intent.setDataAndType(Uri.parse(f"file://{path}"), "resource/folder")
            ...
            print(f"Failed to open Android explorer: {e}")
            return False
