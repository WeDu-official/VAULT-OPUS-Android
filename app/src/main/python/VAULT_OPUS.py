...
async def run_cli(args_list=None):
    ...
    try:
        ...
        if args.command in ["upload", "update"]:
            ...
            if args.modify_command == "move": await ctx.movea(from_path=args.src, to_path=args.dst, DB_FILE=args.database_file, copy_mode=args.copy_mode, id_based=args.id_based)
            elif args.modify_command == "rename": await ctx.renamea(item_path=args.item, new_name=args.new_name, id_based=args.id_based, DB_FILE=args.database_file)
            elif args.modify_command == "makefolder": await ctx.makefoldera(folder_name=args.folder_name, DB_FILE=args.database_file, parent_path=args.parent, id_based=args.id_based)
    finally:
        await bot.close()
        await bot_task
